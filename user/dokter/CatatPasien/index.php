              ...
              <div class="col-md-12 grid-margin">
                <div class="row">
                  <div class="col-12 col-xl-8 mb-4 mb-xl-0">
                    <h3 class="font-weight-bold">Periksa Pasien </h3>
                    <h6 class="font-weight-normal mb-0">Berikut Pasien Anda</h6>
                  </div>
...
$id_dokter = $_SESSION['id_user']; // Sesuaikan dengan sumber ID dokter
$no = 1;
...
$query = "
    ...
    JOIN 
        pasien ps ON dp.id_pasien = ps.id
    WHERE 
        jp.id_dokter = '$id_dokter'
    ORDER BY 
        dp.id ASC
";
...
while ($row = mysqli_fetch_assoc($result)) {
    // Menentukan apakah tombol Periksa atau Sudah Diperiksa
    if ($row['no_antrian'] == 0) {
        $button = "<button class='btn btn-secondary' disabled> Sudah Diperiksa </button>";
    } else {
        // Membuat URL dengan ID daftar poli dan nama pasien
        $button = "<a href='periksa.php?nama_pasien={$row['pasien']}&idp={$row['no']}' class='btn btn-warning'>Periksa</a>";
    }
